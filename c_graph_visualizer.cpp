...
void c_graph_visualizer::draw(){
    ...
    for ( int i  = 0 ; i < n ; i ++){
        ...
        shape.setPosition(300 + 7*G.Koordinaten[i].first,300 + 7*G.Koordinaten[i].second);
        this->window->draw(shape);
        NeighborT N = G.getNeighbors(i);
        for (auto v :N){
            sf::Vertex line[] ={sf::Vertex(sf::Vector2f(shape.getPosition()), sf::Color::Magenta),
            sf::Vertex(sf::Vector2f(300 + 7 * G.Koordinaten[v.first].first,300 + 7 * G.Koordinaten[v.first].second), sf::Color::Magenta) };
